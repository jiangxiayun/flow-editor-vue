...
export default class DragDropResize extends AbstractPlugin {
  ...
  handleMouseMove (event) {
    ...
    if (ORYX_Config.GRID_ENABLED && !modifierKeyPressed) {
    ...
    var checkIfAttachable = this.toMoveShapes.length == 1 && this.toMoveShapes[0] instanceof ORYX_Node && this.toMoveShapes[0].dockers.length > 0
  ...
  onSelectionChanged (event) {
    ...
    if (!elements || elements.length == 0) {
      ...
      if (elements.length == 1 && elements[0].isResizable) {
        let aspectRatio = elements[0].getStencil().fixedAspectRatio() ? elements[0].bounds.width() / elements[0].bounds.height() : undefined
  ...
  snapToGrid (position) {
    ...
    if (offsetX !== undefined) {
      ...
      ul.x = (position.x - (position.x % (ORYX_Config.GRID_DISTANCE / 2)))
      if (this.vLine)
        this.vLine.hide()
    }
    ...
    if (offsetY !== undefined) {
      ...
      ul.y = (position.y - (position.y % (ORYX_Config.GRID_DISTANCE / 2)))
      if (this.hLine)
        this.hLine.hide()
    }
